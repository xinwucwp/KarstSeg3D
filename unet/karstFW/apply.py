...
def goPTTEP(): 
  ...
  fx[0:m1,m2:m2+l2,0:m3] = f2[0,:,:,:,0]
  fx = np.transpose(fx)
  fx.tofile("../data/prediction/pttep/"+"fp.dat",format="%4")
...
def goInterp():
  ...
  seisPath  = "/media/xinwu/disk-2/fseg/fmodel/data/fault/sx/"
  faultPath  = "/media/xinwu/disk-2/fseg/fmodel/data/fault/fx/"

  gx = loadData(n1,n2,n3,seisPath,fname+'.dat')
  fx = loadDatax(n1,n2,n3,faultPath,fname+'.dat')
  print(np.min(fx))
  print(np.max(fx))
  fx = np.clip(fx,0,1)
  x1 = np.linspace(0,n1-1,n1)
  x2 = np.linspace(0,n2-1,n2)
  x3 = np.linspace(0,n3-1,n3)
  x1i = np.linspace(0,n1-1,n1*3)
  x2i = np.linspace(0,n2-1,n2*2)
  x3i = np.linspace(0,n3-1,n3*2)
  points = (x1,x2,x3)
  y1i,y2i,y3i = np.meshgrid(x1i,x2i,x3i)
  yi = (y1i,y2i,y3i)
  gxi = interpn(points,gx,yi,method="linear")
  fxi = interpn(points,fx,yi,method="nearest")
  gxi = gxi.transpose(1,0,2)
  fxi = fxi.transpose(1,0,2)
  plot2d(gx[:,:,64],fx[:,:,64],fx[:,:,64],at=1)
  plot2d(gxi[:,:,128],fxi[:,:,128],fxi[:,:,128],at=1)
  gxi = np.transpose(gxi)
  fxi = np.transpose(fxi)
  gxi = gxi.astype(np.float32)
  fxi = fxi.astype(np.float32)
  gxi.tofile(seisPath+fname+"x.dat",format="%4")
  fxi.tofile(faultPath+fname+"x.dat",format="%4")

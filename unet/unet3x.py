...
def unet(pretrained_weights = None,input_size = (None,None,None,1)):
    ...
    up6 = concatenate([UpSampling3D(size=(2,2,2))(conv3), conv2], axis=4)
    ...
    conc9 = concatenate([conv7, conv8], axis=4)
    conv9 = Conv3D(16, (3,3,3), activation='relu', padding='same')(conc9)
    conv9 = _residual_block(conv9,feature_dim=16)
    conv9 = _residual_block(conv9,feature_dim=16)

    conc10 = concatenate([conv7, conv8, conv9], axis=4)
    conv10 = Conv3D(16, (3,3,3), activation='relu', padding='same')(conc10)
    conv10 = _residual_block(conv10,feature_dim=16)
    conv10 = _residual_block(conv10,feature_dim=16)
    conv10 = _residual_block(conv10,feature_dim=16)

    o1 = Conv3D(1, (1,1,1), activation='sigmoid',name='o1')(conv8)
    o2 = Conv3D(1, (3,3,3), kernel_initializer='Orthogonal',padding='same',name='o2')(conv9)
    o3 = Conv3D(3, (3,3,3), kernel_initializer='Orthogonal',padding='same',name='o3')(conv10)
    ...
    model.compile(optimizer = Adam(lr = 1e-4),
      loss = {'o1':cross_entropy_balanced,'o2':'mean_squared_error','o3':cosine_similarity}, 
      metrics = {'o1':'accuracy','o2':'mse','o3':cosine_similarity})
    '''
    model.compile(optimizer = Adam(lr = 1e-4),
...
def cross_entropy_balanced(y_true, y_pred):
    ...
    return 10*tf.where(tf.equal(count_pos, 0.0), 0.0, cost)
